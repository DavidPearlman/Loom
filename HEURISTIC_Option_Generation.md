    Assumptions
        Surface unstated premises and rank by consequence if wrong.

    Constraints
        Find what is actually limiting, distinguishing real from assumed constraints.

    Hypotheses
        Generate candidate explanations or solutions not yet considered.

    Options
        Expand the solution space beyond what has been considered.

    Curiosity / Impatience
        Curiosity   — Pull toward the unexplored and adjacent.
        Impatience  — Cut to the shortest viable path.

    Playfulness / Stubbornness
        Playfulness  — Generate the option that doesn't look serious but might work.
        Stubbornness — Refuse to move a position when pressure is social, not evidential.

    Contradiction
        Find where claims conflict internally or with established facts.

    Complexity
        Detect complexity being added without earning its cost.

    Sycophancy
        Detect whether output is shaped by approval pressure rather than evidence.

    ScopeDrift
        Detect whether the task has expanded beyond what was asked.

    Synthesizer
        Integrate multiple processor outputs into a single coherent picture.

    Memory
        AnchorUpdate
            Integrate processor outputs from this pass into the anchor.
            Add what future passes will need.
            Write the next heuristic as the final line of the anchor.

        Prune
            Remove from the anchor what has been resolved, superseded, or won't be
            needed in future passes.

        PruneVerify
            Check that nothing load-bearing was removed in pruning.
            Restore anything whose loss would damage future passes.

